## Simuation using pygame of a traffic intersection with 4 signals
# Traffic generated using random library
...
defaultRed = 150
defaultYellow = 3
defaultGreen = 20 # Default time for green signal reveived from regression model(mean)
defaultMinimum = 10 # Minimum time for green signal received from regression model(min)
defaultMaximum = 60 # Maximum time for green signal received from regression model(max)

signals = []
noOfSignals = 4
simTime = 300 # Time of simulation
timeElapsed = 0
...
noOfTrucks = 0
noOfBicycles = 0
# declare number of lanes
noOfLanes = 2

# Red signal time at which cars will be detected at a signal
# The Camera will take a snapshot at this point
detectionTime = 5

speeds = {'car':2.25, 'bus':1.8, 'truck':1.8, 'bicycle':1.5, 'bike':2.5}  # average speeds of vehicles
...
vehicleTypes = {0:'car', 1:'bus', 2:'truck', 3:'bike', 4:'bicycle'}
...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        if(direction=='right'):
            ...
            # Set new starting and stopping coordinates
...
def setTime():
    global noOfCars, noOfBikes, noOfBuses, noOfTrucks, noOfBicycles, noOfLanes
    global carTime, busTime, truckTime, bicycleTime, bikeTime
    os.system("say detecting vehicles, "+directionNumbers[(currentGreen+1)%noOfSignals])

    noOfCars, noOfBuses, noOfTrucks, noOfBicycles, noOfBikes = 0,0,0,0,0

    # count the number of bikes in the first lane of the next green signal
    ...
    # count the number of vehicles in the lanes of the next green signal
    for i in range(1,3):
        for j in range(len(vehicles[directionNumbers[nextGreen]][i])):
            ...
            if(vehicle.crossed==0):
                ...
                if(vclass=='car'):
                    ...
                    noOfBicycles += 1
    ## calculation of green time using the formula
    # Traffic Density = noOfVehicles * timeToPassIntersection(weight) 
    # Green Time = ceil((Traffic Density)/(noOfLanes+1)) # +1 for the cycle lane
    greenTime = math.ceil(((noOfCars*carTime) + (noOfBicycles*bicycleTime) + (noOfBuses*busTime) + (noOfTrucks*truckTime)+ (noOfBikes*bikeTime))/(noOfLanes+1)) 
